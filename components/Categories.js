...
function Categories() {
    ...
    const handleNewSaveButton = () => {
        fetch('https://stockify-backend-one.vercel.app/categories/newCategory', {
            method: 'POST',
    ...
    const CategoryItem = ({ data }) => {
        ...
        const handleSaveButton = (name, id) => {
                fetch(`https://stockify-backend-one.vercel.app/categories/updateMyCategory/${name}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
        ...
        const handleDeleteButton = (name, id) => {
            ...
            if (isConfirmed) {
                fetch(`https://stockify-backend-one.vercel.app/products/productsByCategoryId`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    ...
                    .then(data => {
                        if (data.result == true) {
                            for (let i = 0; i < data.allProducts.length; i++) {
                                fetch(`https://stockify-backend-one.vercel.app/products/updateMyProduct/${data.allProducts[i].name}`, {
                                    method: 'PUT',
                                    headers: { 'Content-Type': 'application/json' },
                    ...
                    .then(fetch(`https://stockify-backend-one.vercel.app/categories/deleteMyCategory/${name}`, {
                        method: 'DELETE',
                        headers: { 'Content-Type': 'application/json' },
                    })
                        .then(() => {
